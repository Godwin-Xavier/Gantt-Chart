...
import { exchangeGithubCodeForProfile } from '../../_lib/oauth.js';
import {
...
export default async function handler(req, res) {
  ...
  if (!code || !state || !verifyOAuthStateFromRequest(req, 'github', String(state))) {
    clearOAuthStateCookie(req, res);
    return redirect(res, withAuthResult(baseUrl, 'error', 'state_mismatch'), 302);
  }

  try {
    const profile = await exchangeGithubCodeForProfile(req, String(code));
